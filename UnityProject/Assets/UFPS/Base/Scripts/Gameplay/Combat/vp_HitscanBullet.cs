...
public class vp_HitscanBullet : MonoBehaviour
{
	...
	void DoHit()
	{
		...
		if (Physics.Raycast(ray, out hit, Range, (IgnoreLocalPlayer ? vp_Layer.Mask.BulletBlockers : vp_Layer.Mask.IgnoreWalkThru)))
            ...
            if (body != null && body.isKinematic) 
            {
                ColliderHit colliderHit = body.GetComponent<ColliderHit>();
                if (colliderHit)
                {
                    colliderHit.AddHitForce(((ray.direction * Force) / Time.timeScale) / vp_TimeUtility.AdjustedTimeScale, hit.point);
                }
            }
