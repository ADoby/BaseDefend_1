		...
		set{
			if (value==true){
				...
				if (state==RagdollState.ragdolled) {
                    //We have to get the parent to the position of the ragdoll
	...
	void LateUpdate()
	{
		...
		if (state==RagdollState.blendToAnim)
		{
			if (Time.time<=ragdollingEndTime+mecanimToGetUpTransitionTime)
			{
				...
				anim.transform.position=newRootPosition;
                ...
                anim.transform.rotation *= Quaternion.FromToRotation(animatedDirection.normalized, ragdolledDirection.normalized);
			}
