...
public class vp_FPController : vp_Component
{
	...
	protected override void Awake()
	{
        ...
        //CharacterController.radius = m_NormalHeight * HeightToRadius;	// NOTE: don't change radius in-game (it may cause missed wall collisions)
	...
	protected override void Start()
	{
		...
		if (PhysicsHasCollisionTrigger)
		{
            ...
            triggerCollider = m_Trigger.AddComponent<CapsuleCollider>();
            triggerCollider.isTrigger = true;
            triggerCollider.radius = CharacterController.radius + m_SkinWidth;
            triggerCollider.height = CharacterController.height + (m_SkinWidth * 2.0f);
	...
	protected virtual void UpdateJumpForceFree()
	{
		...
		else if (Player.Crouch.Active)
		{
			...
			if (Grounded && CharacterController.height == m_NormalHeight)
			{
                ...
                if (triggerCollider)
                {
                    triggerCollider.height = m_CrouchHeight;
                    triggerCollider.center = m_CrouchCenter;
                }
                if (targetCollider)
                {
                    targetCollider.height = m_CrouchHeight;
                    targetCollider.center = m_CrouchCenter;
                }
			}
	...
	protected virtual void OnStop_Crouch()
	{
        ...
        if (triggerCollider)
        {
            triggerCollider.height = m_NormalHeight;
            triggerCollider.center = m_NormalCenter;
        }
        if (targetCollider)
        {
            targetCollider.height = m_NormalHeight;
            targetCollider.center = m_NormalCenter;
        }
	}
